 ...
 this.render = function(canvas, title, height)
  {
   ...
   for(i = 1 ; i <= 10 ; i++)
     {
      jg.drawLine(0,Math.round((h/10*i)),rtmax+20+60,Math.round((h/10*i)));
      //var ff = Math.round((this.max - (this.max / 5 * i))/(this.scale)+this.min);
	var ff = (Math.round(((this.scale)*(10-i)+this.min)*100)/100);
      jg.drawString(ff+"",4,Math.round((h/10*i)-2));
     }
   ...
   var oldx, oldy;
   jg.setStroke(1);
   this.max=(Math.round(((this.scale)*9+this.min)*100)/100);
   for(i = 0; i < this.data.length; i++)
      {
       //var ht1 = Math.round(this.data[i]*h/this.max);// - (this.min*h/this.max);
	 //var ht1 = Math.round((34-33)/0.5*35);// - (this.min*h/this.max);
	var ht1 = Math.round((this.data[i]-this.min)/this.scale*(h/10));// - (this.min*h/this.max);
       ...
       jg.setColor("black");
       jg.drawString(this.x_name[i], sx-2, h);

       oldx = sx;
       oldy = ht1;

       sx = sx+dw+Math.round(dw/2)+shadow+40;
      }
